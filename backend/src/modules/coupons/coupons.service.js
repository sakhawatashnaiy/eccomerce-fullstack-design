...
function validateCouponDoc(coupon, { subtotal }) {
	...
	if (coupon.active === false) return { ok: false, reason: 'Coupon is inactive' }

	const now = new Date()
	if (coupon.expiresAt) {
		const exp = parseDate(coupon.expiresAt)
		if (exp && exp.getTime() < now.getTime()) return { ok: false, reason: 'Coupon is expired' }
	}
	...
	if (amountOff <= 0) return { ok: false, reason: 'Coupon has no discount' }
