...
function normalizePaymentMethod(value) {
    ...
    if (!method)
        return '';
    if (method === 'cash on delivery' || method === 'cash_on_delivery')
        return 'cod';
...
async function createOrderForUser(uid, payload) {
    const { db } = getFirebaseAdmin();
    const { items = [], subtotal = 0, shipping = 0, tax = 0, total = 0, customer = {}, shippingAddress = {}, payment = {}, coupon, couponCode, } = payload || {};
    ...
    if (paymentMethod)
        assertAllowed(paymentMethod, PAYMENT_METHODS, 'payment method');
    ...
    const computedTotal = Math.max(0, computedSubtotal + computedShipping + computedTax - (Number(discount.amount) || 0));
    const order = {
        uid,
        items,
