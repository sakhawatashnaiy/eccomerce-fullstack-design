...
function normalizeImagesInput(images, fallbackImage) {
...
async function resolveProductImages(images) {
...
async function updateProductDoc(id, payload) {
    ...
    const image = payload.image !== undefined ? await resolveProductImage(payload.image) : current.image;
    const resolvedImages = payload.images !== undefined ? await resolveProductImages(payload.images) : current.images;
