...
function isAllowlisted({ uid, email }) {
    const allowedUids = new Set(parseCsv(env_1.env.adminUids));
    const allowedEmails = new Set(parseCsv(env_1.env.adminEmails).map((e) => e.toLowerCase()));
    if (uid && allowedUids.has(String(uid)))
        return true;
...
function getRolesFromDecoded(decoded) {
    ...
    if (decoded?.admin === true)
        roles.add('admin');
    if (isAllowlisted({ uid: decoded?.uid, email: decoded?.email }))
        roles.add('admin');
    if (typeof decoded?.role === 'string')
        addRole(roles, decoded.role);
...
function hasAnyRole(roleSet, allowedRoles) {
    for (const r of allowedRoles) {
        if (roleSet.has(normalizeRole(r)))
            return true;
    }
    return false;
}
function requireRoles(roles = []) {
    ...
    return async function roleGuard(req, _res, next) {
        try {
            ...
            if (hasAnyRole(roleSet, allowed))
                return next();
            // Only hit Firestore if we need it to decide.
            const profile = await (0, auth_service_1.findOrCreateUserProfile)(decoded);
...
function requireSelfOrRoles({ param = 'uid', roles = ['admin'] } = {}) {
    ...
    return async function selfOrRolesGuard(req, res, next) {
        ...
        if (String(req.params?.[param] || '') === String(decoded.uid))
            return next();
