...
function formatDiscountPercent(original, current) {
	const base = Number(original)
	const now = Number(current)
	if (!Number.isFinite(base) || !Number.isFinite(now) || base <= 0 || now >= base) return null
...
export default function ProductDetails() {
	...
	const resolvedVariant = useMemo(() => {
		...
		const match = variants.find((variant) => {
			...
			const colorMatch = selectedColor ? color === selectedColor : true
	...
	const displayStock = resolvedVariant?.stocks ?? product?.stocks ?? 0
	const isOutOfStock = Number(displayStock) <= 0
	const isLowStock = Number(displayStock) > 0 && Number(displayStock) <= 5
	const discountPercent = formatDiscountPercent(compareAt, displayPrice)
	const galleryImages = useMemo(() => {
		...
		variants.forEach((variant) => {
			push(variant?.image)
			push(variant?.options?.image)
		})
		push(resolvedVariant?.image)
		push(resolvedVariant?.options?.image)
		return Array.from(images)
	}, [product?.image, product?.images, variants, resolvedVariant])
	...
	useEffect(() => {
		...
		const defaultVariant = resolvedVariant || variants[0]
		const defaultSize = String(defaultVariant?.options?.size || '').trim()
		const defaultColor = String(defaultVariant?.options?.color || '').trim()
		if (defaultSize && !selectedSize) setSelectedSize(defaultSize)
		if (defaultColor && !selectedColor) setSelectedColor(defaultColor)
	}, [variants, resolvedVariant, selectedVariantId, selectedSize, selectedColor])

	useEffect(() => {
		if (galleryImages.length && !activeImage) setActiveImage(galleryImages[0])
	...
	const handleVariantSelect = (variantId) => {
		...
		if (variant?.options?.size) setSelectedSize(String(variant.options.size))
		if (variant?.options?.color) setSelectedColor(String(variant.options.color))
	}
						...
						<div className="grid gap-8 lg:grid-cols-12">
							...
							<div className="lg:col-span-5">
								<div className="rounded-2xl border border-slate-200 bg-white p-6">
									...
									<VariantSelector
										variants={variants}
										selectedVariantId={resolvedVariant?.id || selectedVariantId}
										...
										<button
											...
											onClick={() =>
												addToCart(product, 1, {
													...
													variantLabel: resolvedVariant?.label || null,
													price: displayPrice,
												})
											}
										>
									...
									<div className="mt-6 rounded-xl bg-slate-50 p-4 ring-1 ring-slate-200">
										...
										<p className="mt-1 text-sm text-slate-600">Delivery in 4-8 days. Free returns within 30 days.</p>
										<p className="mt-2 text-xs text-slate-500">Shipping fee calculated at checkout.</p>
									</div>
							...
							<section className="rounded-2xl border border-slate-200 bg-white p-6">
								...
								{activeTab === 'specs' ? (
									<div className="mt-6 grid gap-3 text-sm text-slate-700 sm:grid-cols-2">
										...
										<div className="rounded-xl bg-slate-50 p-4">
											<p className="text-xs font-semibold text-slate-500">Variant options</p>
											<p className="mt-1 font-semibold text-slate-900">
												{[selectedSize, selectedColor].filter(Boolean).join(' / ') || 'Standard'}
											</p>
										</div>
										<div className="rounded-xl bg-slate-50 p-4">
											<p className="text-xs font-semibold text-slate-500">Availability</p>
											<p className="mt-1 font-semibold text-slate-900">
												{isOutOfStock ? 'Out of stock' : `${displayStock} ready to ship`}
											</p>
										</div>
									</div>
								) : null}
								...
								{activeTab === 'shipping' ? (
									<div className="mt-6 grid gap-4 text-sm text-slate-700 sm:grid-cols-2">
										<div className="rounded-xl border border-slate-200 p-4">
											<p className="text-xs font-semibold text-slate-500">Delivery estimate</p>
											<p className="mt-2 font-semibold text-slate-900">4-8 business days</p>
										</div>
										<div className="rounded-xl border border-slate-200 p-4">
											<p className="text-xs font-semibold text-slate-500">Shipping fee</p>
											<p className="mt-2 font-semibold text-slate-900">Calculated at checkout</p>
										</div>
										<div className="rounded-xl border border-slate-200 p-4">
											<p className="text-xs font-semibold text-slate-500">Returns</p>
											<p className="mt-2 font-semibold text-slate-900">30-day free returns</p>
										</div>
										<div className="rounded-xl border border-slate-200 p-4">
											<p className="text-xs font-semibold text-slate-500">Support</p>
											<p className="mt-2 font-semibold text-slate-900">24/7 seller chat</p>
										</div>
									</div>
								) : null}
							</section>
								...
								<section>
									...
									<div className="mt-4 flex gap-4 overflow-x-auto pb-2">
										{recommendations.map((item) => (
											<div key={item.id} className="min-w-[220px] max-w-[240px] flex-1">
												<ProductCard
													product={item}
